...
export default function EditTweet(param) {
  var { show, close,text,tid,media } = param;
...
useEffect(() => {
  ...
  setIsImage(media.includes("image"))
}, [])
...
const handleEdit = async () => {
  try {
    ...
    const post = await axios.put(CONST.server + `/tweets/${tid}`, formData, {
  ...
  Everyone v
    ...
    />
    { (media!="http://google.com"||file)?isImage?<img style={{width:"100%"}} src={!edit?media:URL.createObjectURL(file)} alt="Selected"/>:<video style={{width:"100%"}} src={!edit?media:URL.createObjectURL(file)} controls  />:null}
